...
Flights1 = Flights1.drop(['Scheduled_Departure','Scheduled_Arrival','Actual_Arrival','Actual_Departure','ELAPSED_TIME','DIVERTED'], axis = 1)
...
X_val=np.array([[3,187,229,69,1.0,35,24.0,3.0,11.0]])
...
X_val=np.array([[12,161,166,0.0,318,2.0,17.0]])
X_val
...
y_val= fitResultdtc.predict(X_val_sc)
y_val
...
X= Flights1.drop(['ELAPSED_TIME','DIVERTED','SCHEDULED_DEPARTURE','CANCELLED','DEPARTURE_DELAY','AIR_TIME','FLIGHT_NUMBER','Is_Delayed'], axis = 1)
...
def percentageofdelay(src,dest,airlinesname):
  ...
  srcno=mapping[src]
  Flights1['DESTINATION_AIRPORT'] = le.fit_transform(Flights1['DESTINATION_AIRPORT'])
  X_test=Flights1.drop(['ELAPSED_TIME','DIVERTED','SCHEDULED_DEPARTURE','CANCELLED','DEPARTURE_DELAY','AIR_TIME','FLIGHT_NUMBER'], axis = 1)
  xgb_from_joblib = joblib.load('drive/My Drive/xgbmodel.pkl')   
  y_pred=xgb_from_joblib.predict(X_test)
